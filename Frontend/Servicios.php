...
<head>
    ...
    <style>
        ...
        /* MARCO DE VIDRIO PARA LA IMAGEN */
        .card-glass-frame {
            position: relative;
            border-radius: 15px;
            overflow: hidden;
        ...
        .new-badge {
            ...
            padding: 2px 15px;
            border-radius: 50px;
            box-shadow: 0 0 15px rgba(206, 17, 38, 0.8);
        ...
        .premium-card:hover .card-reflection {
            left: 150%;
            transition: 0.5s;
        }
    </style>
    ...
    <section class="gallery-premium-section py-5">
        <div class="container">
            <div class="text-center mb-5" data-aos="fade-down">
                <h4 class="modern-title">GALERÍA DE CORTES</h4>
                <div class="title-underline"></div>
            </div>
            <div class="row g-4">
                <?php
                // Consulta para traer los cortes activos, los más nuevos primero
                $sqlGaleria = "SELECT * FROM TBL_BarberGaleria WHERE Estatus = 1 ORDER BY IdCorte DESC";
                $resGaleria = sqlsrv_query($conn, $sqlGaleria);

                while ($row = sqlsrv_fetch_array($resGaleria, SQLSRV_FETCH_ASSOC)):
                    // Armamos la ruta de la imagen
                    $imgRuta = "../Img/Galeria/" . $row['RutaImagen'];
                ?>

                    <div class="col-6 col-md-4 col-lg-3" data-aos="zoom-in">
                        <div class="premium-card">
                            <?php if ($row['IdCorte'] > ($row['IdCorte'] - 4)) {
                                echo '<div class="new-badge">NEW</div>';
                            } ?>

                            <div class="card-glass-frame">
                                <img src="<?= $imgRuta ?>" alt="<?= $row['NombreCorte'] ?>" class="img-fluid">
                                <div class="card-reflection"></div>
                            </div>

                            <div class="premium-info">
                                <span class="barber-name">LA MÉXICO BARBER STUDIO</span>
                                <h4 class="corte-type"><?= $row['NombreCorte'] ?></h4>
                            </div>
                        </div>
                    </div>
